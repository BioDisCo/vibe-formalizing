...
It's important that these files be human readable and inspectable.
Same for every definition (explicit or implicit in the text like computational model): one easily inspectable file for each.
If necessary, create lean libraries.
...
When you're happy with a given proof, add a "verified" badge in the .tex file. If it's correctly formalized, but the proof is missing, add a "formalized" badge. Also add "formalized" badges to key definitions (use https://github.com/BioDisCo/verified-badges). These are the only edits you're allowed to make. Have the badge link to the result's or definition's lean file on GitHub (base URL https://github.com/user/repo/).
